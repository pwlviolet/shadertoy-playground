...
float sdEllipse(vec2 p,vec2 ab)
{
    ...
    return(dot(p/ab,p/ab)>1.)?d:-d;
}

float sdCylinder(in vec3 pos,in vec2 size,in float height,in vec2 scale,in float zoom,in float round_,in float shell,in float hole)
{
    if(hole>0.){
        round_*=(1.-hole*4.);
    }
...
vec2 map(in vec3 pos)
{
    ...
    {
        vec3 d1p=pos;
        float d1=sdCylinder(d1p,vec2(.5,.25),.1,vec2(1.,1.),1.,0.,0.,0.);
        // float d1=sdCylinder(d1p,vec2(.5,.25),.1,vec2(1.,1.),1.,.4,0.,.15);

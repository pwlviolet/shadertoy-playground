...
float fBlob(vec3 p){
    ...
    if(p.x<max(p.y,p.z))p=p.yzx;
    float t1=dot(p,normalize(vec3(1,1,1)));
    float t2=dot(p.xz,normalize(vec2(PHI+1.,1.)));
    float t3=dot(p.yx,normalize(vec2(1.,PHI)));
    float t4=dot(p.xz,normalize(vec2(1.,PHI)));
    float b=max(max(max(t1,t2),t3),t4);
    float l=length(p);

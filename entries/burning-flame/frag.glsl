...
float fire(vec3 p){
    ...
    // float result=geo;
    float velocity=3.;
    float displacement=iTime*velocity;
...
void mainImage(out vec4 fragColor,in vec2 fragCoord)
{
    ...
    vec2 p=centerUv(uv,iResolution.xy);
    p=p*vec2(1.6,-1);
    ...
    vec3 color1=vec3(255.,128.,26.)/255.;
    vec3 color2=vec3(255.,87.,24.)/255.;
    vec3 mixColor=mix(color1,color2,fireBody);
    color*=mixColor;
